...
require_once("catch_errors_taskb.php");
...
$code_p = $curr_xml->xpath("//cCode[.='$cur']/parent::*");
//var_dump($code_p);
...
$old_rate = $rates_xml->xpath("//code[.='$cur']/parent::*");
// var_dump($rates_xml);
// exit;
...
// if the $_GET param $action = post then a new child (currency) of the root node (currencies) is created and then children are added to that node (code, live, rate, cName, country) with all the infomation that the new currency needs 
if ($action == "post"){
...
// if the $_GET param $action = put then the $_GET param $cur is checked against all the the code elements in the currencies file, if they match then all of the code's sibling elements get updated from the currencies file 
}elseif($action == "put"){
